...
Apart from just performing \textit{Maintenance} on the existing codebase,
all other general approaches were already mentioned in section \ref{sect:legacy-software}.
The approach of \textit{Maintenance} is excluded from this subsection as it
has be applied to every software product as shown in section \ref{sect:software-development-process}.
...
\end{figure}\newline
To start from the far left, \textit{Wrapping} is the least revolutionary process.
...
which is then just accessed via a newly created interface i.e. an API.\\
Finally, it is important to discern that it is likely that a combination of
